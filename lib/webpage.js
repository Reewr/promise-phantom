...
class Page {
   ...
   * @param  {number} timeout  How long to wait at maxmium before throwing error, 10 seconds is default
   * @returns {Promise()}
   */
  ...
  render(filename, format, quality) {
    ...
    // UpperCase'd format screws up PhantomJSs guessing of format
    if (!format) {
  ...
  /**
   * Opens a HTML file using a temporary directory - helper
   ...
   */
  _openHtmlInTempDir(htmlString) {
    return new Promise((resolve, reject) => {
      temp.file({postfix: '.html'}, (err, filePath, fd, cleanup) => {
        if (err) {
          return reject(err);
        }

        // PhantomJS does not like absolute paths and will not open it
        // as it follows RFC browser standards. Including file infront
        // lets it open it.
        let URIFilePath = 'file:///' + filePath;

        utils.saveFile(filePath, htmlString)
          .then(() => this.open(URIFilePath))
          .then((status) => {
            cleanup();
            resolve(status);
          }).catch((err) => reject(err));
  ...
  _openHtmlInRenderDir(htmlString, templateRenderDir) {
    return new Promise((resolve, reject) => {
      utils.isDir(templateRenderDir).then((isDir) => {
        if (!isDir) {
        ...
        return utils.saveFile(fname, htmlString)
          .then(() => this.open(fname))
          .then((status) => {
            return utils.deleteFile(fname).then(() => resolve(status));
          });
      }).catch(err => reject(err));
    });
  }
  ...
  openHtml(htmlString, templateRenderDir) {
    ...
    if (typeof templateRenderDir !== 'string') {
      return this._openHtmlInTempDir(htmlString);
    }

    return this._openHtmlInRenderDir(htmlString, templateRenderDir);
  }
  ...
  renderHtml(htmlString, templateRenderDir) {
    ...
    return this.openHtml(htmlString, templateRenderDir).then((status) => {
      if (status !== 'success') {
        throw new Error('Failed to open page correctly');
      }
   ...
   * @returns {Promise(string)}         PDF string
   */
  ...
  openTemplate(template, templateRenderDir, options) {
    ...
    return this.openHtml(html, templateRenderDir);
  }
