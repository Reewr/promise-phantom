 ...
 *
 * @private
 * @param {bool} silenced if true, nothing will be outputted
...
class Page {
  ...
  *_openHtmlInRenderDir(htmlString, templateRenderDir) {
  ...
  /**
   * Saves all the local resouces to a temporary directory
  ...
  *_saveLocalResources(directory) {
   ...
   * @params {string} options.name Unique name, used for retrieval/removal
   * @params {buffer} options.content A buffer of the file content
   * @params {string} options.filename Full filename and directory of
   ...
   *
   * @params {string} name the name of the resource
