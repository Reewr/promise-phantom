...
class Page {
   ...
   *
   * @param {number} framePosition Position of the Frame isnide the Child Frames Array (i.e "window.frames[i]")
  ...
  renderPdf() {
    ...
    return new Promise((resolve, reject) => {
      temp.file({postfix: '.pdf'}, (err, filePath, fd, cleanup) => {
        if (err) {
          return reject(err);
        }
  ...
  _openHtml(htmlString) {
    return new Promise((resolve, reject) => {
      this.set('content', htmlString).then((status) => {
        return resolve(status ? 'success' : 'fail');
      }).catch(reject);
    });
  }
  ...
  openHtml(htmlString, templateRenderDir) {
    ...
    if (typeof templateRenderDir !== 'string') {
      return this._openHtml(htmlString);
    }
