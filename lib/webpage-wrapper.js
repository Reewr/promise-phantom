...
class Page {
   ...
   *   @param {string}   settings.format  Either: 'png', 'gif', 'jpeg', 'pdf'
   *   @param {quality} settings.quality String number value between 0 and 100
   */
  render(filename, settings) {
    ...
    if (typeof format !== 'string' || Page.validRenders.indexOf(format) === -1) {
      let message = 'Settings.format is invalid: "' + format + '". Valid are: ';
    ...
    if (!quality.test(/^[0-9](00|\d)?$/)) {
      throw new TypeError('Settings.quality has to be a number between 0 and 100');
    }
   ...
   *
   * @param {string} format Either 'png', 'gif' or 'jpeg'
  ...
  renderHTMLString(htmlString, templateRenderDir) {
    ...
    return new Promise(function(resolve, reject) {
      utils.generateFilename().then((generatedFilename) => {
